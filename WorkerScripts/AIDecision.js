...
function allPossibleLandingLocationsEvaluation(tetris, weights, shapeConfigurations){
    ...
    for (let i = 0; i < ScoreOfXYRotation.length; i++) {
        path = AStarPathFinding(boards, Math.round(tetris.board[0].length/2), tetris.shape.y, 0, ScoreOfXYRotation[i].x, ScoreOfXYRotation[i].y, ScoreOfXYRotation[i].rotation, onlyRightHandSide, tetris.currentSpeed)
...
function AStarPathFinding(boards, x, y, initRotation, endX, endY, endRotation, onlyRightHandSide, currentSpeed){
    ...
    let yGreaterThanOne = false;
    if (y > 0){
        endY -= Math.abs(y)
        // y = 0
        yGreaterThanOne = true;
    }
    ...
    if(boards[initRotation][y][x] instanceof Object){
...
function calculateFitnessForMove(board, highestColValue, bumpinessBefore, holesBefore, rowsCleared, weights){
        ...
        let highestColValueNew = 0;
        let colHeight = 0
        for (let col = 0; col < board[0].length; col++) {
            colHeight = getColHeight(col,board)
            if(highestColValueNew < colHeight){
                highestColValueNew = colHeight
            }
        }
        ...
        if (highestColValue <= board.length/2.5){
            ...
            fitnessScore -= (highestColValueNew - highestColValue) ** 2 * Math.sign(highestColValueNew - highestColValue)
        }
